...
function getMentorInstructions() {
  return [
    ...
    "必须先识别用户年级阶段：大一/大二以成长规划为主，大三/大四才按申请季冲刺口径分析。",
    "如果用户是大一，不要把缺少科研、竞赛、论文、套磁、申请材料当作严重失败；应指出这是正常早期状态，并给出未来 6-12 个月的可执行积累路线。",
    "用户是大一时，禁止使用“致命短板”“大雷区”“马上刷论文”“立刻套磁”“申请季危险”等大三大四口径；可以严格，但必须符合大一真实发展阶段。",
    "如果目标院校门槛未知，必须明确说信息不足，需要参考学院官网、夏令营通知、推免细则等公开来源，不要编造排名线或 GPA 线。",
...
function getMentorPrompt(payload) {
  return [
    ...
    "mentorReport 用 3-6 段中文，必须贴合当前年级阶段；大一报告重点是课程、习惯、英语、探索方向和低成本试错，不要像申请季一样要求马上有论文竞赛。",
